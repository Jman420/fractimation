...
X_VALUE_INDEX = 0
INITIAL_ELIGIBLE_VERTICES = [[0, 0], [0.5, 1], [1, 0]]

def calculate_subdivisions(vertices):
    first_triangle = vertices[0]
    large_subdivision = (first_triangle[LAST_VERTEX_INDEX, X_VALUE_INDEX] -
                         first_triangle[FIRST_VERTEX_INDEX, X_VALUE_INDEX]) / 2
    small_subdivision = large_subdivision / 2

    left_subdivision_mod = [[0, 0], [-small_subdivision, -large_subdivision],
                            [-large_subdivision, 0]]
    top_subdivision_mod = [[small_subdivision, large_subdivision], [0, 0],
                           [-small_subdivision, large_subdivision]]
    right_subdivision_mod = [[large_subdivision, 0], [small_subdivision, -large_subdivision],
                             [0, 0]]
...
class SierpinskiTriangle(CachedPatchCollectionRenderer):
    ...
    # eligibleVertices is an array of three vertices that describe the points of a triangle as
    #   percentages of screen space.
    # These vertices must be defined from left to right
    #     ie. [ [ left vertex ], [ top vertex ], [ right vertex ] ]
    ...
    def iterate(self):
        ...
        for triangle_vertices in new_subdivisions:
            new_patch = build_triangle(triangle_vertices, line_width)
