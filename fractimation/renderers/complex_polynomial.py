...
class ComplexPolynomial(CachedImageRenderer, ZoomableComplexRange):
    ...
    def iterate(self):
        ...
        # Evaluate Polynomial
        z_values_new = evaluate_polynomial_1d(self._coefficient_array, self._z_values, self._c_value)

        # Update indexes which have exceeded the Escape Value
        exploded_indexes = numpy.abs(z_values_new) > self._escape_value
        self._image_array[self._xIndexes[exploded_indexes], self._yIndexes[exploded_indexes]] = self._next_iteration_index
